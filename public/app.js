    ...
    return `<div class="attention-banner" role="alert"><span><strong>${blockedMembers.length}件のチャットが対応待ちです</strong><small>${guidance}</small></span><button id="focusBlocked" class="sm" type="button">${actionLabel}</button></div>`;
      ...
      <div class="compile">
        ...
        ${workspace.lastCompile
          ? `<hr><div class="compile-result-head"><div class="compile-result-meta"><strong>作成済み</strong><span class="small">${esc(displayTimestamp(workspace.lastCompile.at))}</span><span class="small">直近回答 ${members.filter((member) => member.messages.some((message) => message.role === 'assistant')).length}件を使用</span></div><div class="compile-result-actions"><button id="copyCompile" class="sm" type="button">結果をコピー</button><button id="downloadCompile" class="sm" type="button">Markdownで保存</button></div></div><div class="compile-output" id="compileOutput" role="region" aria-label="統合レポートの結果" tabindex="0">${renderCompileText(workspace.lastCompile.text)}</div>`
