...
- **Regeneration:** `generate_mcp_token` Tauri command overwrites Keychain; the running MultiContext child must be restarted (Desktop does this on next `保存して開始` or app relaunch) to pick up the new env. Copy actions may temporarily expose the full token in clipboard — this is intentional for one-time paste into the client config.
...
- If user binds beyond localhost (`MULTICONTEXT_HOST=0.0.0.0`), they intentionally expose MCP; document warns about token exposure and recommends tunnel.
